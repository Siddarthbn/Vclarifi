...
# --- Data Access (No changes needed here) ---
@st.cache_data
def fetch_organization_data(user_email):
    ...
    try:
        ...
        st.info("Please ensure your database credentials in `.streamlit/secrets.toml` are correct and the database is running.")
        return None, None
    finally:
        if conn and conn.is_connected(): conn.close()

# --- Recommendation Generation (No changes needed here) ---
...
def generate_recommendations(_category_name, average_score, questions_context):
    try:
        ...
        st.info("Please ensure your Gemini API key in `.streamlit/secrets.toml` is correct.")
...
def recommendations_page(navigate_to=None, user_email=None):
    ...
    # REFINED: Main heading is now inside its own styled container
    st.markdown(f"""
